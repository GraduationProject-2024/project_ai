...
def fill_form():
    ...
    location = request.form.get("incident_location")
    if not location:
    ...
    content_ko, content_en = summarize_content(content)  #STT 후 요약 & 필터링 자동 적용
    processed_content = f'{content_en}({content_ko})'

    default_title_ko, default_title_en, default_emergency_type = generate_title_and_type(content)

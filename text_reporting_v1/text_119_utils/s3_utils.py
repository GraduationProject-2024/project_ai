...
def upload_image_to_s3(image_file):
    """
    이미지를 AWS S3에 업로드하여 URL 반환 (Twilio MMS용)
    ...
    # 파일 객체가 아닌 경우 (예를 들어 문자열이 전달된 경우) 에러 메시지 출력 후 None 반환
    if not hasattr(image_file, "filename"):
        print("업로드할 파일 객체가 아닌 값이 전달되었습니다:", image_file)
        return None

    file_name = f"images/{int(datetime.now().timestamp())}_{image_file.filename}"  # S3 경로 설정
    
    s3_client.upload_fileobj(image_file, S3_BUCKET_NAME, file_name)

    image_url = f"https://{S3_BUCKET_NAME}.s3.amazonaws.com/{file_name}"
    print("image_url:", image_url)
    return image_url

    # folder = "images/"  # 이미지 저장 폴더

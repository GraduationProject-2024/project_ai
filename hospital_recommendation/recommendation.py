...
class HospitalRecommender:
    ...
    def train_autoencoder(data, input_dim, latent_dim, epochs=50, lr=0.001):
        """
        AutoEncoder 학습
        """
    ...
    # 사용자 프로필 임베딩 함수
    def embed_user_profile(basic_info, health_info):
        """
        사용자 프로필을 경량화된 모델 및 수치형 데이터로 벡터화
        """
        text_data = " ".join([
    ...
    # 병원 데이터 임베딩 함수
    def embed_hospital_data(hospitals_df, suspected_disease=None):
        """
        병원 데이터를 텍스트 임베딩 및 수치형 데이터로 벡터화
        """
        # 진료과 텍스트 데이터 벡터화
        dgsbjt_embeddings = self.embedding_model.encode(hospitals_df["dgsbjt"].fillna("").tolist())

        # 병원 유형 텍스트 임베딩
        clcdnm_embeddings = self.embedding_model.encode(hospitals_df["clcdnm"].fillna("").tolist())

        # 병원 이름 텍스트 임베딩 추가
        yadmnm_embeddings = self.embedding_model.encode(hospitals_df["yadmnm"].fillna("").tolist())
        ...
        if suspected_disease:
            ...
            suspected_disease_embeddings = np.zeros((hospitals_df.shape[0], dgsbjt_embeddings.shape[1]))

        # 최종 병원 벡터 결합
        return np.hstack((dgsbjt_embeddings, clcdnm_embeddings, yadmnm_embeddings, suspected_disease_embeddings, time_embedding))
    ...
    # 추천 함수
    def recommend_hospitals(autoencoder, user_embedding, hospital_embeddings, hospitals_df, department=None, suspected_disease=None, use_autoencoder=False):
        """
        사용자 임베딩과 병원 임베딩의 유사도를 계산하여 추천
        department가 병원 이름에 포함된 경우 가중치 추가
        """
        ...
        # department 유사도 추가 (선택적)
        if department:
            # 병원 이름에 department 포함 여부 확인 및 가중치 부여
            hospitals_df["department_match"] = hospitals_df["yadmnm"].apply(lambda name: department in name)
            hospitals_df["similarity"] += hospitals_df["department_match"] * 0.1  # 가중치 0.05 추가    

        # suspected_disease와 병원 이름 유사도 추가 (선택적)
        if suspected_disease:
            disease_embeddings = self.embedding_model.encode([suspected_disease])
            hospital_name_embeddings = self.embedding_model.encode(hospitals_df["yadmnm"].fillna("").tolist())
            disease_similarities = cosine_similarity(disease_embeddings, hospital_name_embeddings)[0]
            hospitals_df["similarity"] += disease_similarities * 0.001  # 가중치 0.0001 추가
